...
def get_response_stream(session_id, user_question, result_df, Responder_model='gemini-1.0-pro'):
    """Generates a streaming natural language response based on SQL query results.

    This generator function creates a ResponseAgent and yields text chunks as they are 
    generated by the underlying LLM. It handles session history for question rewriting
    similar to get_response().

    Args:
        session_id (str): Session ID to identify the chat conversation.
        user_question (str): The user's natural language question.
        result_df (str): The results of the SQL query in JSON format.
    ...
    Raises:
        Exception: If there's an error during response generation, yields an error message.
    """
    try:
        Responder = ResponseAgent(Responder_model)
        ...
        for chunk in Responder.run_stream(user_question, result_df):
            yield chunk

    except Exception as e:
        print(f"An error occurred during streaming. Error Message: {e}")
        yield f"Error has been encountered :: {str(e)}"
...
############################
##_____RUN PIPELINE STREAM_____##
############################
async def run_pipeline_stream(session_id,
                user_question,
                user_grouping,
                RUN_DEBUGGER=True,
                EXECUTE_FINAL_SQL=True,
                ...
                example_similarity_threshold = 0.3, 
                num_sql_matches=3): 
    """Streaming version of run_pipeline that yields response chunks.

    This async generator orchestrates the end-to-end SQL generation and response pipeline,
    but streams the natural language response as it's generated by the LLM.
    ...
    Args:
        ...
        EXECUTE_FINAL_SQL (bool, optional): Whether to execute the final SQL query. Defaults to True.
        DEBUGGING_ROUNDS (int, optional): The number of debugging rounds to perform. Defaults to 2.
        LLM_VALIDATION (bool, optional): Whether to use LLM for validation. Defaults to False.
        Embedder_model (str, optional): The name of the embedding model. Defaults to 'vertex'.
        SQLBuilder_model (str, optional): The name of the SQL builder model. Defaults to 'gemini-1.5-pro'.
        SQLChecker_model (str, optional): The name of the SQL checker model. Defaults to 'gemini-1.0-pro'.
        SQLDebugger_model (str, optional): The name of the SQL debugger model. Defaults to 'gemini-1.0-pro'.
    ...
    Yields:
        dict: Dictionary containing different types of data:
            - {"type": "sql", "data": str, "session_id": str} - The generated SQL query
            - {"type": "results", "data": str} - SQL execution results as JSON
            - {"type": "text", "data": str} - Streaming text chunks of the response
            - {"type": "error", "data": str} - Error message if something fails
            - {"type": "done"} - Indicates streaming is complete
    """

    # Generate SQL (non-streaming)
    final_sql, session_id, invalid_response = await generate_sql(
        session_id,
        user_question,
        user_grouping,
        RUN_DEBUGGER,
    ...
    if invalid_response:
        yield {"type": "error", "data": str(final_sql)}
        yield {"type": "done"}
        return

    # Yield the generated SQL
    yield {"type": "sql", "data": final_sql, "session_id": session_id}

    # Execute SQL and get results
    if EXECUTE_FINAL_SQL:
        results_df, exec_invalid = get_results(
            user_grouping,
            final_sql,
            invalid_response=False,
            EXECUTE_FINAL_SQL=EXECUTE_FINAL_SQL
        ...
        if exec_invalid:
            yield {"type": "error", "data": str(results_df)}
            yield {"type": "done"}
            return

        # Yield the SQL results
        yield {"type": "results", "data": results_df.to_json(orient='records')}

        # Stream the natural language response
        try:
            for chunk in get_response_stream(session_id, user_question, results_df.to_json(orient='records'), Responder_model=Responder_model):
                yield {"type": "text", "data": chunk}
        except Exception as e:
            yield {"type": "error", "data": str(e)}
    else:
        yield {"type": "results", "data": "SQL execution disabled"}
